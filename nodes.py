...
def base_augment(access_token, output_dir, limit_opus_free, ignore_errors, req_type, image, options=None):
    ...
    try:
        ...
        print(f"[NovelAI] Anlas tracking failed (pre-augment): {e}")
    ...
    try:
        ...
        if start_anlas is not None:
            try:
                ...
                print(f"[NovelAI] Anlas tracking failed (post-augment): {e}")
...
class ColorizeAugment:
    ...
    def augment(self, image, limit_opus_free, ignore_errors, defry, prompt):
        return base_augment(self.access_token, self.output_dir, limit_opus_free, ignore_errors, "colorize", image,
...
class EmotionAugment:
    ...
    def INPUT_TYPES(s):
        return {"required": {"image": ("IMAGE",),
                             "limit_opus_free": ("BOOLEAN", {"default": True, "tooltip": TOOLTIP_LIMIT_OPUS_FREE}),
                             "ignore_errors": ("BOOLEAN", {"default": False}), "mood": (
                ["neutral", "happy", "sad", "angry", "scared", "surprised", "tired", "excited", "nervous", "thinking",
                 ...
                 "worried", "love", "determined", "hurt", "playful"], {"default": "neutral"}),
    ...
    def augment(self, image, limit_opus_free, ignore_errors, mood, strength, prompt):
        ...
        return base_augment(self.access_token, self.output_dir, limit_opus_free, ignore_errors, "emotion", image,
                            options={"defry": defry, "prompt": prompt})
...
class V4CharacterPromptStack:
    ...
    def INPUT_TYPES(s):
        ...
        for i in range(1, 6):
            required[f"char_{i}_caption"] = ("STRING", {"multiline": True, "default": "", "tooltip": f"Character {i} Positive"})
            required[f"char_{i}_negative"] = ("STRING", {"multiline": True, "default": "", "tooltip": f"Character {i} Negative"})
            required[f"char_{i}_x"] = ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.1, "display": "number"})
            required[f"char_{i}_y"] = ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.1, "display": "number"})
